...
// ฟังก์ชันสร้างกล่องข้อความอิสระ (เพิ่มใหม่)
function insertFloatingTextBox() {
    saveState();
    const t = getVisiblePage(); 
    if (!t) { showDialog("กรุณาเพิ่มหน้ากระดาษก่อนครับ"); return; }
    
    const w = document.createElement('div'); 
    // ทริก: ใส่ class 'post-it' เข้าไปด้วย เพื่อให้ระบบดึงระบบยืดขยายขนาด (Width/Height) มาใช้อัตโนมัติ
    w.className = 'smart-object selected post-it floating-text-box'; 
    w.style.top = (40 + Math.random()*10) + '%'; 
    w.style.left = (35 + Math.random()*10) + '%'; 
    w.style.width = '250px'; 
    w.style.minHeight = '100px'; 
    w.style.backgroundColor = 'rgba(255, 255, 255, 0.9)'; // พื้นหลังสีขาวโปร่งแสงนิดๆ
    w.style.boxShadow = '0 4px 15px rgba(0,0,0,0.05)';
    w.style.borderRadius = '8px'; 
    w.style.padding = '16px';
    w.style.border = '2px dashed #cbd5e1'; // กรอบเส้นประสีเทา
    
    w.innerHTML = `
        <div class="content-editable-area outline-none text-gray-700 text-sm leading-relaxed w-full h-full overflow-auto" contenteditable="true">
            พิมพ์ข้อความอิสระที่นี่...<br>✨ ดึงมุมขวาล่างเพื่อปรับขนาดกล่องได้เลย!
        </div>
        <div class="controls"><div class="delete-handle">✕</div><div class="resize-handle"></div></div>
    `; 
    t.appendChild(w); 
